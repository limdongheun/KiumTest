...
namespace KOASampleCS
{
    public partial class Form1 : Form
    {
        ...
        public void ChangeTradeList(string Name, int nState, string sOrderNo, int nBuyQty, int nBuyPrice, int nSellQty, int nSellPrice)
        {
            ...
            for (int i = 0; i < 1000; i++)
            {
                if (stTradeData.sName[i] == Name)
                {
                    ...
                    if (nState <= 3)
                    {
                        if(stTradeData.nState[i] == 2 && nState == 3)
                        {
                        ...
                        if (stTradeData.nState[i] == 15 && nState == 3)
                        {
                        ...
                        if (stTradeData.nState[i] == 18 && nState == 3)
                        {
                            stTradeData.nState[i] = 20;
                            stTradeData.nBuyQty[i] += nBuyQty;
                            stTradeData.nBuyPrice[i] = nBuyPrice;
                            stTradeData.nBuyTime[i] = nNowTime;
                    ...
                    else if (nState > 3)
                    {
                        ...
                        if ((stTradeData.nState[i] == 35 || stTradeData.nState2[i] == 47 || stTradeData.nState2[i] == 53) && nState == 6)
                        {
                            stTradeData.nSellQty2[i] += nSellQty;
                            stTradeData.nSellPrice2[i] = nSellPrice;
                            stTradeData.bUnder910[i] = false;
                        ...
                        if ((stTradeData.nState[i] == 4 || stTradeData.nState[i] == 5 || stTradeData.nState[i] == 22 || stTradeData.nState[i] == 24) && nState == 6)
                        {
        ...
        public void TradeDataCheck1()
        {
            try
            {
                while (true)
                {
                    ...
                    for (int i = 0; i < 1000; i++)
                    {
                        if(stTradeData.sCode[i] != "" && stTradeData.nState[i] != 100)
                        {
                            ...
                            if (stTradeData.nState[i] != 1 && stTradeData.nPivot2[i] > 0 && stTradeData.nMStartPrice[i, 0] == 0 && m_bNextDayChcek == true && m_bNextMinChcek == true && nNowTime > 901)
                            {
                                if (nNowTime < 1500)
                                {
                                    //stTradeData.nState[i] = 1;

                                    stTradeData.nMStartPrice[i, 0] = 1;
                            ...
                            else if (stTradeData.nState[i] == 3 && nNowTime >= 905)
                            {
                                ...
                                LogManager.WriteLine("시초 돌파매도 : " + stTradeData.sCode[i] + "\t" + stTradeData.sName[i] + "\t" + stTradeData.nNowPrice[i].ToString());
                            }
                            else if (stTradeData.nState[i] == 4 && nNowTime >= 910)
                            {
                                stTradeData.nState[i] = 5;

                                int lRet = SendOrder(stTradeData.sCode[i], stTradeData.nOrderQty[i], 3, "00", 0, stTradeData.sOrderNo[i]);
                                LogManager.WriteLine("매도정정 :\t" + stTradeData.sCode[i] + "\t" + stTradeData.sName[i]);
                            ...
                            else if (stTradeData.nState[i] == 14)
                            {
                                stTradeData.nState[i] = 15;
                            ...
                            else if (stTradeData.nState[i] == 30 && nNowTime < 1400 && stTradeData.nBuyPrice[i] * 1.025 < stTradeData.nNowPrice[i])
                            {
                                ...
                                int lRet = SendOrder(stTradeData.sCode[i], nQty, 2, "07", 0, "");
                                LogManager.WriteLine("돌파 매도(2.5%) : " + stTradeData.sCode[i] + "\t" + stTradeData.sName[i] + "\t" + stTradeData.nNowPrice[i].ToString());
                            }
                            else if (stTradeData.nState[i] == 30 && nNowTime >= 1400 && stTradeData.nBuyPrice[i] * 1.013 < stTradeData.nNowPrice[i])
                            {
                                stTradeData.nState[i] = 31;
                                ...
                                int lRet = SendOrder(stTradeData.sCode[i], nQty, 2, "07", 0, "");
                                LogManager.WriteLine("돌파 매도(1.3%) : " + stTradeData.sCode[i] + "\t" + stTradeData.sName[i] + "\t" + stTradeData.nNowPrice[i].ToString());
                            }
                            else if (stTradeData.nState[i] == 30 && stTradeData.nSellTime[i] < nNowTime && stTradeData.nBuyPrice[i] < stTradeData.nNowPrice[i])
                            {
                                int sellTime = stTradeData.nSellTime[i] + 30;
                            ...
                            else if (stTradeData.nState[i] == 30 && stTradeData.nSellTime[i] < nNowTime && stTradeData.nBuyPrice[i] * 0.98 > stTradeData.nNowPrice[i])
                            {
                                ...
                                LogManager.WriteLine("시간초과 매도 : " + stTradeData.sCode[i] + "\t" + stTradeData.sName[i] + "\t" + stTradeData.nNowPrice[i].ToString());
                            }
                            else if (stTradeData.nState[i] == 16)
                            {
                                stTradeData.nState[i] = 18;
                                ...
                                LogManager.WriteLine("급상승(2%) : " + stTradeData.sCode[i] + " 기준 : " + stTradeData.nStandardPrice[i].ToString() + " 현재가 : " + stTradeData.nNowPrice[i].ToString());
                            ...
                            else if (stTradeData.nState[i] == 21)
                            {
                                stTradeData.nState[i] = 22;

                                int nQty = stTradeData.nOrderQty[i];

                                int lRet = SendOrder(stTradeData.sCode[i], nQty, 2, "07", 0, "");
                                LogManager.WriteLine("급상승 매도 : " + stTradeData.sCode[i] + "\t" + stTradeData.sName[i] + "\t" + stTradeData.nNowPrice[i].ToString());
                            }
                            else if (stTradeData.nState[i] == 23)
                            {
                                stTradeData.nState[i] = 24;
            ...
            try
            {
                ...
                else if (e.sRQName == "주식일봉차트조회")
                {
                    ...
                    if (nCnt > 0)
                    {
                        ...
                        if(stTradeData.nStandardPrice[nCodeCount] < stTradeData.nPivot1[nCodeCount])
                        {
                            stTradeData.nStandardPrice[nCodeCount] = stTradeData.nPivot1[nCodeCount];
                        }
                ...
                else if (e.sRQName == "주식분봉차트조회")
                {
                    ...
                    if (nCnt > 0)
                    {
                        ...
                        string sNowTime = System.DateTime.Now.ToString("HHmm");
                        int nNowTimeCount = (System.DateTime.Now.Hour - 9) * 60 + System.DateTime.Now.Minute;
                        if (nNowTimeCount > 0)
                        {
                            nNowTimeCount = nNowTimeCount / 5;
                        }
                        ...
                        for (int i = 0; i < nCnt; i++)
                        {
                            ...
                            else
                            {
                                ...
                                if(nNowTimeCount-1 > nTimeCount)
                                {
                                    if(nHighPrice < stTradeData.nMHighPrice[nCodeCount, nTimeCount])
                                    {
                                        nHighPrice = stTradeData.nMHighPrice[nCodeCount, nTimeCount];
                                    }
                                }
        ...
        private void axKHOpenAPI_OnReceiveRealData(object sender, AxKHOpenAPILib._DKHOpenAPIEvents_OnReceiveRealDataEvent e)
        {
            ...
            for (int i = 0; i < 1000; i++)
            {
                if (stTradeData.sCode[i] == e.sRealKey && stTradeData.nState[i] != 100)
                {
                    ...
                    if (sNowPrice != "" && nNowTime >= 900)
                    {
                        ...
                        if (stTradeData.nMTime[i, nTimeCount] == 0)
                        {
                            ...
                            else if (stTradeData.nState[i] == 12)
                            {
                            ...
                            else if(stTradeData.nState[i] == 13 && stTradeData.nNowStartPrice[i] > stTradeData.nMLowPrice[i, nTimeCount - 1])
                            {
                                stTradeData.nState[i] = 100;
                            }
                            ...
                            else if (stTradeData.nState[i] == 20)
                            {
                                stTradeData.nState[i] = 21;
                            }
                            else if (stTradeData.nState[i] == 22)
                            {
                                stTradeData.nState[i] = 23;
                            }
                        ...
                        */
                        ...
                        else if (nNowTime > 910 && stTradeData.nState2[i] < 51 && stTradeData.nPivot1[i] > 0 && stTradeData.nNowPrice[i] > stTradeData.nPivot1[i] && stTradeData.nStandardPrice[i] > 0 && stTradeData.nStandardPrice[i] < stTradeData.nNowPrice[i])
                        {
                            stTradeData.nState2[i] = 51;
